...
def placeBlock(x, z):
    """Place un bloc de laine noire aux coordonnée (x, height, z)"""
    global height
    global mc
...
def init_frame(angle, center, radius):
    """Initialise les valeurs utiles pour l'interprétation de l'image
        ...
        center : (x, y), centre de la plateforme de construction en pixel sur l'image.
...
def buildPositions(n, bordure, largeurTotale):
    """Crée la liste des tuples sauvegardant les positions à vérifier sur l'image et leur position correspondante dans mineraft.
...
#Point de l'image qui restera inchangé entre chaque manipulation de l'utilisateur. Est utilisé pour limiter les problèmes liés à la luminosité
pointToWatch = (3, 30)

#Liste de tuples sauvegardant les positions à vérifier sur l'image et leur position correspondante dans mineraft. 
#tuple : (mcx, mcy, x, y)
# x : pixel x 
# y : pixel y
# mcx : minecraft x
...
#Temps en secondes durant lequel le bouton doit rester pressé avant que le système ne se réinitialise.
resetPushTime = 5
...
def onButtonPressed():
    ...
    g = g1-g2
    b = b1-b2
    # Vérifie pour chaque couleur à chaque position définie par build_positions() si la différence de couleur est plus grande que le seuil de tolérence. 
    ...
    for (mcx, mcy, x, y) in positions:
        if checkDif(B1, B2, x, y, b) or
            ...
            #Si la différence dépasse la tolérence pour on moins une couleur, place un bloc dans minecraft aux coordonnées correspondantes.
            placeBlock(mcx, mcy)
...
def checkDif(img1, img2, x, y, dif2, tolerence=70):
    """Regarde si la différence d'intensité de couleur au points (x, y) entre img1 et img2 dépasse la tolérence.
        -dif2 : La différence d'intensité entre last et new au point qui ne devrait pas changer. Permet d'être moins dépendant d'une luminosité constante.  
"""
    pos1 = int(img1[x, y])
    pos2 = int(img2[x, y])
    dif = pos1 - pos2 - dif2
...
for f in camera.capture_continuous(rawCapture,
    ...
    #Affiche l'image initialisée, permet de vérifier que les valeurs entrée lo7rs de la configuration sont justes.
    if new_pic != None and i < 200:
